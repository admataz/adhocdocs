...
describe('schema', () => {
  describe('create schema', () => {
    it('should create a new schema from a valid payload', done => {
      request.post({ url: `${baseUrl}/api/v1/schema`, body: testSchema.validSchema, json: true }, (err, res, body) => {
        if (err) throw (err)
    ...
    it('should not allow duplicate schema IDs', done => {
      request.post({ url: `${baseUrl}/api/v1/schema`, body: testSchema.validSchema, json: true }, (err, res, body) => {
        if (err) throw (err)
        request.post({ url: `${baseUrl}/api/v1/schema`, body: testSchema.validSchema, json: true }, (err, res2, body) => {
          if (err) throw (err)
    ...
    it('should reject an invalid schema', done => {
      request.post({ url: `${baseUrl}/api/v1/schema`, body: testSchema.invalidSchema, json: true }, (err, res, body) => {
        if (err) throw (err)
    ...
    it('should reject a schema with no name property defined', done => {
      request.post({ url: `${baseUrl}/api/v1/schema`, body: testSchema.missingName, json: true }, (err, res, body) => {
        if (err) throw (err)
        expect(res.statusCode).equal(400)
        done()
      })
  ...
  describe('update schema', () => {
    it('should update an existing schema with new properties', done => {
      request.post({ url: `${baseUrl}/api/v1/schema`, body: testSchema.validSchema, json: true }, (err, res, body) => {
        if (err) throw (err)
        request.put({ url: `${baseUrl}/api/v1/schema/${testSchema.validSchema._id}`, body: testSchema.updateSchemaBody, json: true }, (err, res, body) => {
          if (err) throw (err)
  ...
  describe('get schema', () => {
    it('should get an existing schema by schema id', done => {
      request.post({ url: `${baseUrl}/api/v1/schema`, body: testSchema.validSchema, json: true }, (err, res, body) => {
        if (err) throw (err)
        request.get({ url: `${baseUrl}/api/v1/schema/${testSchema.validSchema._id}`, json: true }, (err, res, body) => {
          if (err) throw (err)
    ...
    it('should return a 404 for a non-existing schema id', done => {
      request.get({ url: `${baseUrl}/api/v1/schema/non-existent-schema`, json: true }, (err, res, body) => {
        if (err) throw (err)
    ...
    it('should get a list of existing schema by schema', done => {
      request.post({ url: `${baseUrl}/api/v1/schema`, body: testSchema.validSchema, json: true }, (err, res, body) => {
        ...
        request.post({ url: `${baseUrl}/api/v1/schema`, body: validSchema2, json: true }, (err, res, body) => {
          if (err) throw (err)
          request.get({ url: `${baseUrl}/api/v1/schema`, json: true }, (err, res, body) => {
            if (err) throw (err)
